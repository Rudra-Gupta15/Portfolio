...
export default function Skills() {
  ...
  useEffect(() => {
    ...
    (function draw(){
      ...
      for(let li=0;li<nodes.length-1;li++)for(const a of nodes[li])for(const b of nodes[li+1]){const w=(a.act+b.act)/2;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=`rgba(212,168,67,${.08+w*.1})`;ctx.lineWidth=.35+w*.5;ctx.stroke();}
